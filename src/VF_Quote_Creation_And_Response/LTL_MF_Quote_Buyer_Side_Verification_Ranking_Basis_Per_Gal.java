...
public class LTL_MF_Quote_Buyer_Side_Verification_Ranking_Basis_Per_Gal {
	...
	public void setUp(){
		...
		htmlReporter= new ExtentHtmlReporter("../Veri-Fuel_Hybrid_Framework/LTL Mobile Fueling/LTL_Quote_Buyer_Side_Calculations_Verification_$_Per_Gal.html");
	...
	public  void Verify_$_Per_Gal_Values_in_Workbench() throws Exception{
		...
		ExtentTest test1 = extent.createTest("Verify_$_Per_Gal_Values_in_Workbench", "Verify and Validated $/Gal values in Workbench");
		test1.log(Status.INFO, "Verify and Validated $/Gal values in Workbench");

		try {

			for(int i=1,j=39,k=24; i<=2&&j<=39&&k<=25; i++,j--,k++){
				...
				if(actual$perdel.equalsIgnoreCase(excepted$perdel)){
					...
					System.out.println("$/Gal for " +ExcelUtils.getCellData(k, colnum)+ " category Value is validated in Workbench as "+ $perdel.getText());
					test1.pass("$/Gal for " +ExcelUtils.getCellData(k, colnum)+ " category Value is validated in Workbench as "+ $perdel.getText());

				}

				else
				{
					System.out.println("$/Gal for " +ExcelUtils.getCellData(k, colnum)+ " category Value is not validated in Workbench as "+ $perdel.getText());
					test1.fail("$/Gal for " +ExcelUtils.getCellData(k, colnum)+ " category Value is not validated in Workbench as "+ $perdel.getText());


				}
			}

			WebElement total$delvalue=driver.findElement(By.id("CstperperiodftPlh"));
			
			String expectedtotal$perdel=ExcelUtils.getCellData(54, colnum);
			String actualtotal$perdel=total$delvalue.getText();
			
			if(actualtotal$perdel.equalsIgnoreCase(expectedtotal$perdel)){

				System.out.println("Total $/Gal Value is validated in Workbench as "+ total$delvalue.getText());
				test1.pass("Total $/Gal Value is validated in Workbench as "+ total$delvalue.getText());

			}

			else
			{
				System.out.println("Total $/Gal Value is not validated in Workbench as "+ total$delvalue.getText());
				test1.fail("Total $/Gal Value is not validated in Workbench as "+ total$delvalue.getText());
...
*/
	...
	@Test(priority =29)
	public  void Verify_$_Per_Gal_Values_in_Workbench_for_Awarded_quote() throws Exception{

		//Verify $/Del Values in Workbench

		ExtentTest test1 = extent.createTest("Verify_$_Per_Gal_Values_in_Workbench_for_Awarded_quote", "Verify and Validated $/Gal values in Workbench for Awarded Quote");
		test1.log(Status.INFO, "Verify and Validated $/Gal values in Workbench for Awarded Quote");
