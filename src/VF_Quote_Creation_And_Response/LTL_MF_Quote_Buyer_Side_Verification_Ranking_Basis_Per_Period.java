...
public class LTL_MF_Quote_Buyer_Side_Verification_Ranking_Basis_Per_Period {
	...
	public void setUp(){
		...
		htmlReporter= new ExtentHtmlReporter("../Veri-Fuel_Hybrid_Framework/LTL Mobile Fueling/LTL_Quote_Buyer_Side_Calculations_Verification_$_Per_Period.html");
	...
	public  void Verify_$_Per_Period_Values_in_Workbench() throws Exception{
		...
		//Verify $/Period Values in Workbench

		ExtentTest test1 = extent.createTest("Verify_$_Per_Period_Values_in_Workbench", "Verify and Validated $/Period values in Workbench");
		test1.log(Status.INFO, "Verify and Validated $/Period values in Workbench");

		try {

			for(int i=1,j=41,k=24; i<=2&&j<=41&&k<=25; i++,j--,k++){

				WebElement $perdel=driver.findElement(By.xpath("//*[@id='ProductsWorkbenchGrid']/div[3]/table/tbody/tr["+i+"]/td[18]"));
				...
				if(actual$perdel.equalsIgnoreCase(excepted$perdel)){
					...
					System.out.println("$/Period for " +ExcelUtils.getCellData(k, colnum)+ " category Value is validated in Workbench as "+ $perdel.getText());
					test1.pass("$/Period for " +ExcelUtils.getCellData(k, colnum)+ " category Value is validated in Workbench as "+ $perdel.getText());

				}

				else
				{
					System.out.println("$/Period for " +ExcelUtils.getCellData(k, colnum)+ " category Value is not validated in Workbench as "+ $perdel.getText());
					test1.fail("$/Period for " +ExcelUtils.getCellData(k, colnum)+ " category Value is not validated in Workbench as "+ $perdel.getText());


				}
			}

			WebElement total$delvalue=driver.findElement(By.id("CstperiodftPlh"));
			
			String expectedtotal$perdel=ExcelUtils.getCellData(57, colnum);
			String actualtotal$perdel=total$delvalue.getText();
			
			if(actualtotal$perdel.equalsIgnoreCase(expectedtotal$perdel)){

				System.out.println("Total $/Period Value is validated in Workbench as "+ total$delvalue.getText());
				test1.pass("Total $/Period Value is validated in Workbench as "+ total$delvalue.getText());

			}

			else
			{
				System.out.println("Total $/Period Value is not validated in Workbench as "+ total$delvalue.getText());
				test1.fail("Total $/Period Value is not validated in Workbench as "+ total$delvalue.getText());
...
*/
	...
	public  void Verify_Final_Total_And_Ranking_Basis_in_TandC_for_Awarded_Quote() throws Exception{
		...
		try {
			...
			WebElement finaltotal=driver.findElement(By.id("ldlfinaltotal"));
			BigDecimal val = new BigDecimal(finaltotal.getText());
			DecimalFormat decimalFormat = new DecimalFormat("#,###.##");
			String numberAsString = decimalFormat.format(val);
			
			System.out.println("Final Total value is " + numberAsString);
			test1.info("Final Total value is " + numberAsString);
	...
	@Test(priority =29)
	public  void Verify_$_Per_Period_Values_in_Workbench_for_Awarded_quote() throws Exception{

		//Verify $/Del Values in Workbench

		ExtentTest test1 = extent.createTest("Verify_$_Per_Period_Values_in_Workbench_for_Awarded_quote", "Verify and Validated $/Period values in Workbench for Awarded Quote");
		test1.log(Status.INFO, "Verify and Validated $/Period values in Workbench for Awarded Quote");
